...
(function() {
  /**
   * Obsolete function for compatibility.
   */
  AudioContext.prototype.createJavaScriptNode = function() {

    return this.createScriptProcessor.apply(this, arguments);
  }
  ...
  Tuner = function() {
    ...
    $(window).resize(function() {
      ...
      return canvas.width = $('.tuner').width() + 600;
    });
...
*/
          ...
          var displayNote = $('#displayedNote');


          var indicator1 = (diff<-6),
          		indicator2 = (diff>-6 && diff <= -4),
          		indicator3 = (diff>-4 && diff <= -2),
          		indicator4 = (diff>=2 && diff < 4),
          		indicator5 = (diff>=4 && diff < 6),
          		indicator6 = (diff>=6);
          var displayedStep = $('#displayedStep');

          // update note display here
          var noteStep = { '#' : 'sharp', 'b' : 'flat'};
          displayNote.attr({ 'class' : 'digital ' + note.substr(0,1) });
          displayedStep.attr({ 'class' : ((note.length === 2) ? 'digital ' + noteStep[note.substr(1,1)] : 'digital na' ) });

	        // change opacity based on if note is in tune or out of tune
          displayNote.css({ "opacity" : ((Math.abs(diff) < 1.99) ? 1.0 : 0.6) });

          // update bar indicators
          if (indicator1) {
	          $('ul#mainDisplay li > div ').attr({'class':'glow'});
	          $('ul#mainDisplay li.indicator:lt(3) > div.glow').addClass('active');

          } else if (indicator2) {
	          $('ul#mainDisplay li > div ').attr({'class':'glow'});
	          $('ul#mainDisplay li.indicator:eq(2) > div.glow').addClass('active');
	          $('ul#mainDisplay li.indicator:eq(1) > div.glow').addClass('active');
	          ...
	          $('ul#mainDisplay li > div ').attr({'class':'glow'});
	          $('ul#mainDisplay li.indicator:eq(3) > div.glow').addClass('active');

          } else if (indicator5) {
	          $('ul#mainDisplay li > div ').attr({'class':'glow'});
	          $('ul#mainDisplay li.indicator:eq(3) > div.glow').addClass('active');
	          $('ul#mainDisplay li.indicator:eq(4) > div.glow').addClass('active');

          } else if (indicator6) {
	          $('ul#mainDisplay li > div').attr({'class':'glow'});
	          $('ul#mainDisplay li.indicator:gt(2) > div.glow').addClass('active');
          ...
          return displayDiv.html(''); //return displayDiv.html(displayStr);
        ...
        clear: function() {
          ...
          // update note display here
          $('#displayedNote').attr({ 'class' : 'digital' });
	        $('#displayedStep').attr({ 'class' : 'digital' });
        }
      };
      render = function() {
